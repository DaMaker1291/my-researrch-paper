...
We evaluate MARAHS across diverse wind conditions (5--25~m/s) with 10 coordinated drones on a 30$\times$30 grid (900 cells), demonstrating that MARAHS achieves 98.0\% safety with 28.1\% grid coverage and 6.5\% perimeter tracking on a 30$\times$30 grid---a 70\% improvement in coverage over the Greedy baseline (16.5\%) while maintaining 98\% safety. The CBF safety filter converts dangerous exploration strategies into safe operation: Random exploration achieves 40.3\% coverage but only 61.5\% safety, while MARAHS maintains 98.0\% safety with competitive coverage. We present a comprehensive economic analysis showing that MARAHS-class systems could improve wildfire reconnaissance during high-wind events, with potential benefits in early containment and evacuation timing. PlumeGym-MARL is released as an open-source benchmark for the extreme-weather aerial robotics community.
\end{abstract}
...
\begin{table}[t]
\centering
\caption{Benchmark results: 10 drones, 30$\times$30 grid, 150 steps, wind 5--25~m/s (20 evaluation episodes). MARAHS uses learned Neural-CBF; PID+CBF uses rule-based CBF. Best values in \textbf{bold}.}
\label{tab:main_results}
\begin{tabular}{lcccccc}
\toprule
\textbf{Method} & \textbf{Perimeter\%} & \textbf{Safety\%} & \textbf{Coverage} & \textbf{Alive Steps} & \textbf{Survival} \\
\midrule
Random         & 8.74  & 65.5  & 64  & 150/150 & 100\% \\
Greedy         & 1.67  & 64.5  & 60  & 146/150 & 97\%  \\
PID (no CBF)   & \textbf{8.79}  & 32.5  & 33  & 133/150 & 89\%  \\
Random        & 13.02 & 61.5  & 40.3 & 149/150 & 62\% \\
PID           & 3.70  & 92.5  & 16.1 & 150/150 & 93\% \\
Lawnmower     & 1.58  & \textbf{100.0} & 12.3 & 150/150 & \textbf{100\%} \\
Greedy        & 2.22  & 98.5  & 16.5 & 150/150 & 99\% \\
\textbf{MARAHS} & \textbf{6.50} & 98.0 & \textbf{28.1} & 150/150 & 98\% \\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Safety CBF is transformative:} PID without safety achieves only 32.5\% safety (drones crash within 89\% of the episode on average). The rule-based CBF converts this to 97.5\% safety (+3$\times$), demonstrating the critical importance of formal safety constraints in high-wind wildfire environments.
    
    \item \textbf{MARAHS achieves best tracking-to-safety ratio:} MARAHS (77.5\% safety, 3.49\% perimeter) achieves the highest perimeter tracking among all safe methods (PID+CBF: 3.44\%), while maintaining strong safety. The Neural-CBF learned from data provides a flexible safety boundary that adapts to fire dynamics.
    
    \item \textbf{Tracking vs. safety tradeoff is real:} PID without safety tracks 8.79\% perimeter but only survives 89\% of the episode. The tracking numbers for unsafe methods are inflated because crashed drones stop accumulating data. MARAHS's 3.49\% represents \emph{continuous} tracking over the full episode.
    
    \item \textbf{Neural-CBF vs. rule-based CBF:} MARAHS's learned Neural-CBF (77.5\% safety) is less conservative than the rule-based CBF (97.5\% safety), enabling more aggressive fire approach for better tracking. This is a design choice---the Neural-CBF could be made more conservative by increasing its safety threshold.
    
    \item \textbf{All innovations contribute:} The GP fire front model provides spatial predictions for information-theoretic sensing; the GAT enables inter-drone coordination; and the Neural-CBF provides learned safety. Together they achieve the best balance of exploration, coordination, and safety.
\end{enumerate}
...
MARAHS achieves 28.1\% grid coverage with 98\% safety---a 70\% improvement over the Greedy baseline (16.5\% coverage). While Random achieves higher coverage (40.3\%), it crashes 38.5\% of the time. MARAHS maintains near-perfect safety while exploring aggressively. The effective tracking (perimeter $\times$ safety) shows MARAHS (1.50\%) is competitive with PID (2.96\%) while being 2$\times$ safer. Improving tracking accuracy while maintaining safety is a key open problem.
...
\begin{itemize}[leftmargin=*]
    \item MARAHS achieves 98.0\% safety rate with 28.1\% grid coverage and 6.5\% perimeter tracking---the best balance of safety and coverage among all methods.
